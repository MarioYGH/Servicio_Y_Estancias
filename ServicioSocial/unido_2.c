...
#include "sdmmc_cmd.h"
#include "driver/sdmmc_types.h" /* This library is necessary to deal with the SD card via SPI. Change line 181 at sdmmc_types.h from 20000 to 5000 */
#include "driver/gpio.h"
...
esp_err_t write_displacements2sd() {
    ...
    ds1307_get_time(&rtc_dev, &time); // Leer la hora desde el RTC

    if(gpio_get_level(BOOL_SENSOR_1) || gpio_get_level(BOOL_SENSOR_2) || gpio_get_level(BOOL_SENSOR_3)) {
        strcpy(data, "");
        snprintf(data, sizeof(data), "%04d-%02d-%02d %02d:%02d:%02d,%i,%i,%i\n", 
...
void app_main(void) {
    ...
    ESP_ERROR_CHECK(ds1307_init_desc(&rtc_dev, 0, DS1307_SDA_GPIO, DS1307_SCL_GPIO));
