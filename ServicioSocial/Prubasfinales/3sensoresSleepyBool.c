...
void app_main(void){
    ...
    while(1){
        ...
        for (int i = 0; i < N_SENSORS; i++) {
            if (sensor_states[i]) { // Check if the sensor should be read
                /* Read Sensor */
                get_adc_value(adc1_handle, (adc_channel_t)(ADC1_CHAN7 - i * 3), &adc_raws[i]);
                adc_voltages[i] = adc_raws[i] * 3.3 / 4095.0;
                calculate_distance(i, adc_voltages[i]);
                ESP_LOGI(TAG, "Sensor %d Distance: %.2f mm", i + 1, distances[i]);
            } else {
                ESP_LOGI(TAG, "Sensor %d is disabled", i + 1);
            }
        }
